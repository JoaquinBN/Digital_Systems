...
static int row;
static unsigned char timerBounces,timer_SMS;
...
void initKeypad(void){
    ...
    TRISBbits.TRISB6 = 0;
    TRISBbits.TRISB7 = 0;
    TI_NewTimer(&timerBounces);
    TI_NewTimer(&timer_SMS);
...
void keypadLoop(void){
    ...
    switch (state){
        ...
        case 4:
            state=5;
            TI_ResetTics(timerBounces);
            break;
        case 5:
            if (TI_GetTics(timerBounces) >= 32){
                if (!!(PORTBbits.RB3 && PORTBbits.RB1 && PORTBbits.RB5)){
                    state = 0;
                } else{
                    pressed = 1;
    ...
    switch(stateSMS){
        ...
        case 0:
            ...
            if(pressed){
                ...
                available = 1;
                if(numPresses != 0 && (previous != currentKey || TI_GetTics(timer_SMS) >= 2000)){
                    available = 0;
                    previous = currentKey;
                    numPresses = 0;
...
char KeGetCharValue(void){
    char generic = KeGetGenericValue();
    if ((generic - '0' >= 2 && generic - '0' <= 9) || (generic - '0' == 0)){
        if(generic - '0' == 0){
            if(numPresses > 2){
                numPresses = 1;
            }
        ...
        }else if (SMSTABLE[generic - '0' - 1][(numPresses-1)] == '\0'){
            numPresses = 1;
        }
