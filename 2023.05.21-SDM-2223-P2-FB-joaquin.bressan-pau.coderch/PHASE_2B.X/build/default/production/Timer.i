...
unsigned char TI_NewTimer(unsigned char *TimerHandle) {
 ...
 while (Timers[Counter].Busy==1) {
  if (++Counter == 15) return (0);
 }
 Timers[Counter].Busy=1;
 *TimerHandle=Counter;
    return (1);
}

void TI_ResetTics (unsigned char TimerHandle) {
 (INTCONbits.GIE = 0); Timers[TimerHandle].InitialTics=Tics; (INTCONbits.GIE = 1);
}

unsigned long TI_GetTics (unsigned char TimerHandle) {
    (INTCONbits.GIE = 0); unsigned long CopyCurrentTics=Tics; (INTCONbits.GIE = 1);
 return (CopyCurrentTics-(Timers[TimerHandle].InitialTics));
}

void TI_CloseTimer (unsigned char TimerHandle) {
 Timers[TimerHandle].Busy=0;
}
